...
KEY_FROM_NAME = re.compile(r"(?:Key|key|path|filename|filepath|destination)\s*[:=]\s*(?:[^;{\n]{0,90}"
...
ACCEPT_SVG = re.compile(r"(?:===|==)\s*['\"]image/svg\+xml['\"]|\b(?:accept|allow|isAllowed)[a-z_0-9]*\s*[:=(][^;]{0,90}?(?:image/svg\+xml|['\"]svg['\"])", re.I)
...
SERVE_FILE = re.compile(r"res\.sendFile|\.sendFile\s*\(|proxyMedia|streamObject|\.pipe\s*\(\s*(?:res|w)\b", re.I)
