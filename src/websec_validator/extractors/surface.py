...
SINKS = {
    "ssrf": ("ssrf-probes", None, re.compile(
        r"(?:\bfetch|axios(?:\.\w+)?|got|node-fetch|superagent(?:\.\w+)?|needle(?:\.\w+)?|undici(?:\.\w+)?|requests\.\w+|httpx\.\w+|urllib\.request\.\w+)"
        r"\s*\(\s*[^)\n;]{0,160}?" + _REQ_SRC)),
    ...
    "ssrf-outbound-http": ("ssrf-probes", None, re.compile(
        r"(?:axios(?:\.(?:get|post|put|delete|patch|request|head))?|got|node-fetch|needle(?:\.(?:get|post|put|delete|patch|request|head))?|superagent(?:\.(?:get|post|put|delete|patch|request|head))?|undici(?:\.(?:request|fetch))?"
