...
| V2 | HIGH is reserved for verified evidence (dynamic-confirmed, verified secret, HIGH-CVE); recon-only output never emits HIGH. The ordinal is already semi-binary: LOW/MEDIUM = heuristic strength, HIGH = verified. | `docs/METHODOLOGY.md:174-179` (the published confidence rule); V1(b) sites |
...
| V13 | Labelled assets on disk: `tests/test_detector_precision.py` (26 paired vulnerable/sanitised cases), 13 fixture directories under `tests/fixtures/` in clean/poisoned and present/missing pairs, and `calibration.samples_from_dynamic` which labels **only** `bola` from controlled probes. `bug-212` records why the local overlay was poisoned once and why v2 quarantines old-schema overlays: the rationale is in-source at `calibration.py:297-302`, the quarantine itself is `_upgrade_local` (`calibration.py:247-255`), it is pinned by `tests/test_calibration_claimspec.py:134`, and the fix is recorded in `CHANGELOG.md` under `[0.13.0]`. | file census; source read; `CHANGELOG.md` [0.13.0] |
