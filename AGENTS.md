...
| `python3 -m unittest discover -s tests` | Run the suite (103 tests, stdlib only, no network) |
...
2. **After changing code or docs**: re-run `docguard guard`; keep the numbers (15 extractors, 14 sink
   classes, 103 tests, 10/10 proof) consistent across every doc — DocGuard's metrics-consistency
