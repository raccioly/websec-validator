...
| `python3 -m unittest discover -s tests` | Run the suite (135 tests, stdlib only, no network) |
...
2. **After changing code or docs**: re-run `docguard guard`; keep the numbers (16 extractors, 15 sink
   classes, 135 tests, 10/10 proof) consistent across every doc — DocGuard's metrics-consistency
